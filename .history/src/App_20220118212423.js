...
const App = () => {
  ...
  const handleCalculations = () => {
      const totalMoneyCount = items.reduce((total, item) => {
        const totalPrice = item.quantity * item.price
        return total + totalPrice;
      }, 0);
      
  setTotalMoneyCount(totalMoneyCount);
  };
  // To do : check if selected then work
  return (
    <div className="app-background">
      <div className="main-container">
        ...
        <div className="total">Total Price: {totalMoneyCount}</div>
      </div>
    </div>
  );
};
